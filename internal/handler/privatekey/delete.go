...
// @Success 200 {object} response.SuccessResponse "Private key deleted successfully"
...
func (h *PrivateKeyHandler) DeletePrivateKey(w http.ResponseWriter, r *http.Request) {
	...
	// Check if the private key exists before deleting
	...
	if err = repository.DeletePrivateKeyByID(r.Context(), tx, privateKeyID, teamID); err != nil {
		zap.L().Error("Failed to delete private key", zap.Error(err))
		response.RespondWithError(w, http.StatusInternalServerError, "Failed to delete private key", "FAILED_TO_DELETE_PRIVATE_KEY")
		return
	}
